...
function convertColumnToEditable(value, row, column, vm, index) {
    /*console.log(vm, row, index);*/
    var id = row.id;
    var value = value;
    var column = column;

    var isEditingRow = (vm.editingRow[id] != undefined) ? true : false;
    console.log(isEditingRow);


    console.log(vm);
    var html = "<span v-if='!isEditingRow(\""+id+"\")' @dblclick='showEditRow(\""+id+"\")'>" + value + "</span>";
    html += "<input type='text' v-if='isEditingRow(\""+id+"\")' class='form-control' @keydown.enter='edit("+id+", \""+column+"\", $event)' value='" + value + "'>"
