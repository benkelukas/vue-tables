...
function convertColumnToEditable(value, row, column, vm) {
    var id = row.id;
    var value = value;
    var column = column;
    ...
    if (selectColumn !== undefined) {//we got select column as editable
        ...
        html += "<select name='" + column + "' class='form-control' " +
            "v-show='isEditingRow(\"" + id + "\", \"" + column + "\")' " +
            "@blur='edit(" + id + ", \"" + column + "\", $event)'>";
    ...
    var html = "<span v-if='!isEditingRow(\"" + id + "\", \"" + column + "\")' @dblclick='showEditRow(\"" + id + "\", \"" + column + "\")'>" + value + "</span>";
    html += "<input type='text' v-if='isEditingRow(\"" + id + "\", \"" + column + "\")' class='form-control' " +
        "@keydown.esc.stop='cancelEdit(" + id + ", \"" + column + "\")'" +
        "@keydown.enter='edit(" + id + ", \"" + column + "\", $event)' value='" + value + "'" +
        ">"
...
module.exports = function (value, row, column, rowIndex) {
    ...
    if (this.editableColumns.indexOf(column) != -1) {
        return convertColumnToEditable(value, row, column, vm);
    }
